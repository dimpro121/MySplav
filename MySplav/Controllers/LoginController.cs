...
namespace MySplav.Controllers
{
    public class LoginController : Controller
    {
        ...
        {
            var result = await LoginDomain.API.VerifyUserAsync(model.Email, model.Password, _dc);
            if (result.User != null)
            {
        ...
        {
            var result = await LoginDomain.API.RegistrationUserAsync(model.Email, model.Password, _dc);
            if (result.User != null)
            {
                await SignInUser(result.User);
            }
